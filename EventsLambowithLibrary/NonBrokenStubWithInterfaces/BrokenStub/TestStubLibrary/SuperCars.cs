...
{
    public abstract class SuperCars : ILaunchable, ILiftable
    {
        ...
        public string EngineCheck()
        {
            // is the engine on? 
            //Is it running 
            return $"You have a {engine.Name}  your engine is currently {engine.IsStarted}  and your {IsEngineRunning()} ";
        ...
        {
            //spec of car
            return $" You are drive a {this.Name} with a {SetEngineName(this.Name)} engine which top speed is set at {TopSpeed} mph with a {transmission.HowManyGears} transmission";
        }
        ...
        public string ActivateLift(ILiftable Islifted)
        {
            if (IsLifted == true)
            {
                return $"Your {this.Name} is being lifted to clear any obstacles ";
            }
            else
            {
                return "your car is not being lifted";
